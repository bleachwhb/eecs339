...
my $inputLat = cookie("Lat");
my $inputLong = cookie("Long");
#
...
my $action;
my $run;
my $countcommitteetotal;
my $countindividualtotal;
my $countcommitteedem;
my $countcommitteerep;
my $countindividualdem;
my $countindividualrep;
my $opcoloravg;
my $opcolorstd;
...
# height=1024 width=1024 id=\"info\" name=\"info\" onload=\"UpdateMap()\"></iframe>";
   ...
   eval
   {
   @cycless=ExecSQL($dbuser,$dbpasswd,"select distinct cycle from cs339.committee_master order by cycle","COL");
   };
   my @dataplus;
   if(UserCan($user,"give-opinion-data"))
{
   print start_form(-name=>'Option'),
   p('cycle:'),
   checkbox_group
  (
  -name=>'cyclechoice',
...
if($countcommitteedem>=$countcommitteerep)
{
  print "<div id=\"colorc1\" style=\"width:100\%; height:10\%\">Committees: The amount of money involved by the committees in the current view is : \$$countcommitteetotal\n,including \$$countcommitteedem\nfor Democratic Party,\$$countcommitteerep\nfor Republican Party</div>";
}
if($countcommitteedem<$countcommitteerep)
{
print "<div id=\"colorc2\" style=\"width:100\%; height:10\%\">Committees: The amount of money involved by the committees in the current view is : \$$countcommitteetotal\n,including \$$countcommitteedem\nfor Democratic Party,\$$countcommitteerep\nfor Republican Party</div>";    
}
...
if($countindividualdem>=$countindividualrep)
{
  print "<div id=\"colori1\" style=\"width:100\%; height:10\%\">Individuals: The amount of money involved by the Individuals in the current view is : \$$countindividualtotal\n,including \$$countindividualdem\nfor Democratic Party,\$$countindividualrep\nfor Republican Party</div>";
}
if($countindividualdem<$countindividualrep)
{
print "<div id=\"colori2\" style=\"width:100\%; height:10\%\">Individuals: The amount of money involved by the individuals in the current view is : \$$countindividualtotal\n,including \$$countindividualdem\nfor Democratic Party,\$$countindividualrep\nfor Republican Party</div>";    
}
...
if($opcoloravg>=0.5)
{
print "<div id=\"coloro1\" style=\"width:100\%; height:10\%\">Opinions: the average of color is :\$$opcoloravg\n, and the standerd deviation of color is :\$$opcolorstd\n</div>";
}
if($opcoloravg<0.5&&$opcoloravg>=-0.5)
{
print "<div id=\"coloro2\" style=\"width:100\%; height:10\%\">Opinions: the average of color is :\$$opcoloravg\n, and the standerd deviation of color is :\$$opcolorstd\n</div>";
}
if($opcoloravg<-0.5)
{
print "<div id=\"coloro3\" style=\"width:100\%; height:10\%\">Opinions: the average of color is :\$$opcoloravg\n, and the standerd deviation of color is :\$$opcolorstd\n</div>";
}
...
if ($action eq "invite-user") { 
	...
	if (!$run){
		...
		p('Please choose which permissions you want the invited user to have:'),
       		checkbox_group(
			-name=>'permissions_choices',
			-values => \@permissions,
		),
 		hidden(-name=>'run',-default=>['1']),
		...
		my @permissions_choices=param('permissions_choices');
                my $perm = join(',',@permissions_choices);
		#my $sendPermissions = join(','@permission_choices);
		my $error;
		$error = UserInvite($email,$perm);
		if ($error){
			print "Can not invite a user because: $error";
		}else{
			print "E-mail sent";
			print $email;
                        print $perm;
...
sub Committees {
  ...
  my $sum2=0;
  my $maxcount=0;
 while($count<$countmin&&$maxcount!=3)
{
 #    eval
 #   {
 #      @totalmoney=ExecSQL($dbuser,$dbpasswd,"select sum(transaction_amnt),count(transaction_amnt) from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_comm where cycle in ($cycle) and latitude>?  #and latitude<? and longitude>? and longitude<?","ROW",$latsw,$latne,$longsw,$longne);
 #    };

     eval
    {
      @totalmoney=ExecSQL($dbuser,$dbpasswd,"select sum(transaction_amnt),count(transaction_amnt) from (select* from cs339.committee_master where cycle in ($cycle)) natural join cs339.comm_to_comm natural join (select* from cs339.cmte_id_to_geo where latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne)","ROW");
     };

     eval
    {
       @demmoney=ExecSQL($dbuser,$dbpasswd,"select sum(transaction_amnt),count(transaction_amnt) from (select* from cs339.committee_master where cycle in ($cycle) and cmte_pty_affiliation in ('DEM','dem','d','D','Dem')) natural join cs339.comm_to_comm natural join (select* from cs339.cmte_id_to_geo where latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne)","ROW");
     };
     eval
    {
       @repmoney=ExecSQL($dbuser,$dbpasswd,"select sum(transaction_amnt),count(transaction_amnt) from (select* from cs339.committee_master where cycle in ($cycle) and cmte_pty_affiliation in ('REP','rep','r','R','Rep')) natural join cs339.comm_to_comm natural join (select* from cs339.cmte_id_to_geo where latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne)","ROW");
     };


#   eval
#  {
 #      @demmoney=ExecSQL($dbuser, $dbpasswd, "select sum (transaction_amnt), count(transaction_amnt) from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_comm where cycle in ($cycle) and latitude>? #and latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('DEM','dem','d','D','Dem')","ROW",$latsw,$latne,$longsw,$longne);
 #   };
#     eval
#   {
 #   @repmoney=ExecSQL($dbuser, $dbpasswd, "select sum (transaction_amnt) , count(transaction_amnt) from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_comm where cycle in ($cycle) and latitude>? #and latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('REP','rep','r','R','Rep')","ROW",$latsw,$latne,$longsw,$longne);
#  };
...
$countcommitteetotal=$sum;
$countcommitteedem=$sum1;
$countcommitteerep=$sum2;

#if($sum1>=$sum2)
#{
#  print "<div id=\"colorc1\" style=\"width:100\%; height:10\%\">'Committees': The amount of money involved by the committees in the current view is : \$$sum\n,including \$$sum1\nfor Democratic Party,\$$sum2\nfor Republican Party</div>";
#}
#if($sum1<$sum2)
#{
#print "<div id=\"colorc2\" style=\"width:100\%; height:10\%\">'Committees': The amount of money involved by the committees in the current view is : \$$sum\n,including \$$sum1\nfor Democratic Party,\$$sum2\nfor Republican Party</div>";    
#}
...
#      eval
#    {
#       @totalmoney=ExecSQL($dbuser, $dbpasswd, "select sum (transaction_amnt) , count(transaction_amnt) from cs339.individual natural join cs339.cmte_id_to_geo natural join cs339.committee_master where cycle in ($cycle) and #latitude>? and latitude<? and longitude>? and longitude<? ","ROW",$latsw,$latne,$longsw,$longne);
#   };
#   eval
#   {
#      @demmoney=ExecSQL($dbuser, $dbpasswd, "select sum(transaction_amnt), count(transaction_amnt) from cs339.individual natural join cs339.cmte_id_to_geo natural join cs339.committee_master where cycle in ($cycle) and latitude>? and #latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('DEM','dem','d','D','Dem')","ROW",$latsw,$latne,$longsw,$longne);
 #  };
 #    eval
 #  {
 #  @repmoney=ExecSQL($dbuser, $dbpasswd, "select sum (transaction_amnt) , count(transaction_amnt) from cs339.individual natural join cs339.cmte_id_to_geo natural join cs339.committee_master where cycle in ($cycle) and latitude>? and #latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('REP','rep','r','R','Rep')","ROW",$latsw,$latne,$longsw,$longne);
#  };

 eval
    {
      @totalmoney=ExecSQL($dbuser,$dbpasswd,"select sum(transaction_amnt),count(transaction_amnt) from (select* from cs339.committee_master where cycle in ($cycle)) natural join cs339.individual natural join (select* from cs339.cmte_id_to_geo where latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne)","ROW");
     };

     eval
    {
       @demmoney=ExecSQL($dbuser,$dbpasswd,"select sum(transaction_amnt),count(transaction_amnt) from (select* from cs339.committee_master where cycle in ($cycle) and cmte_pty_affiliation in ('DEM','dem','d','D','Dem')) natural join cs339.individual natural join (select* from cs339.cmte_id_to_geo where latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne)","ROW");
     };
     eval
    {
       @repmoney=ExecSQL($dbuser,$dbpasswd,"select sum(transaction_amnt),count(transaction_amnt) from (select* from cs339.committee_master where cycle in ($cycle) and cmte_pty_affiliation in ('REP','rep','r','R','Rep')) natural join cs339.individual natural join (select* from cs339.cmte_id_to_geo where latitude>$latsw and latitude<$latne and longitude>$longsw and longitude<$longne)","ROW");
     };
  ...
  $countindividualtotal=$sum;
  $countindividualdem=$sum1;
  $countindividualrep=$sum2;

#if($sum1>=$sum2)
#{
#  print "<div id=\"colori1\" style=\"width:100\%; height:10\%\">'Individuals': The amount of money involved by the committees in the current view is : \$$sum\n,including \$$sum1\nfor Democratic Party,\$$sum2\nfor Republican Party</div>";
#}
#if($sum1<$sum2)
#{
#print "<div id=\"colori2\" style=\"width:100\%; height:10\%\">'Individuals': The amount of money involved by the individuals in the current view is : \$$sum\n,including \$$sum1\nfor Democratic Party,\$$sum2\nfor Republican Party</div>";    
#}
...
my @opinioncolor;
  eval { 
    @opinioncolor = ExecSQL($dbuser, $dbpasswd, "select avg(color), stddev(color) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?","ROW",$latsw,$latne,$longsw,$longne);
  };
    $opcoloravg=$opinioncolor[0];
    $opcolorstd=$opinioncolor[1];
...
sub UserInvite{
	...
	my $invitelink = "http://murphy.wot.eecs.northwestern.edu/~bwj4163/rwb/rwb.pl?act=register&&rid=$randomid";
...
sub GiveOpinions{
	...
	my $latne = param("latne");
	my $longne = param("longne");
	eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_opinions (submitter, color, latitude, longitude) values (?,?,?,?)",undef,$_[1],$_[0],$inputLat ,$inputLong) ;};
